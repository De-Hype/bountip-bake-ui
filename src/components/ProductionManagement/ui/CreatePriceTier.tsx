...
const CreatePriceTier: React.FC<CreatePriceTierProps> = ({ onSave, onClose }) => {
  const [tier, setTier] = useState<PriceTier>({ name: "", description: "", markupPercent: undefined, discountPercent: undefined });
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setTier({ name: "", description: "", markupPercent: undefined, discountPercent: undefined });
  ...
  return (
    ...
    >
      <div className="relative bg-white rounded-xl shadow-lg w-full max-w-md mx-4 p-8">
        ...
        <h2 className="text-xl font-semibold mb-6 text-center">Create Price Tier</h2>
        <div className="space-y-4">
            ...
            {markupEnabled && (
              <input
                ...
                onChange={(e) =>
                  setTier({
                    ...tier,
                    markupPercent: e.target.value === "" ? undefined : Math.max(0, Math.min(100, parseFloat(e.target.value)))
                  })
                }
            ...
            {discountEnabled && (
              <input
                ...
                onChange={(e) =>
                  setTier({
                    ...tier,
                    discountPercent: e.target.value === "" ? undefined : Math.max(0, Math.min(100, parseFloat(e.target.value)))
                  })
                }
          ...
          <button
            ...
            type="button"
            disabled={loading || !tier.name.trim() || (!markupEnabled && !discountEnabled)}
          >
