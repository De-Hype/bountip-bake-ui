...
const CreateProductModals: React.FC<CreateProductModalsProps> = ({
  ...
  useEffect(() => {
    if (outletsData?.outlet.priceTier) {
      ...
      setFormData((prev) => ({
        ...prev,
        priceTiers: availablePriceTiers.map((tier: any) => ({
          id: tier.id,
          name: tier.name,
          description: tier.description,
          pricingRules: tier.pricingRules,
          isActive: tier.isActive,
          label: tier.name, // Add required 'label' property
          price: tier.price ?? "", // Add required 'price' property, default to empty string if missing
